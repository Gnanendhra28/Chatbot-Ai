...
import {
  MessageSquare, Play, XCircle, Trash2, Search,
      ...
      <div className="surface-card overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-40 gap-2">
            ...
            <Link href="/chat" className="btn btn-primary mt-2 text-xs"><Plus size={12} /> Start one</Link>
          </div>
        ) : (
          <table className="w-full">
            <thead>
              <tr style={{ borderBottom: "1px solid var(--border)" }}>
                {["Title", "Status", "Provider", "Messages", "Tokens", "Updated", ""].map((h) => (
                  <th
                    key={h}
                ...
                >
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium truncate max-w-[240px]" title={conv.title}>
                        {conv.title}
                      </span>
                    </div>
                    <p className="text-xs mt-0.5 font-mono ml-5" style={{ color: "var(--text-muted)" }}>
                      #{conv._id.slice(-8)}
                    </p>
                  </td>
                  <td className="px-4 py-3">
                    <span className={`tag ${STATUS_CONFIG[conv.status]?.class ?? "tag-muted"}`}>
                      {conv.status}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    ...
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      {conv.status === "active" && (
                        <button
                          ...
                          title="Cancel"
                        >
                          <XCircle size={13} style={{ color: "var(--warning)" }} />
                        </button>
                      )}
                      <button
