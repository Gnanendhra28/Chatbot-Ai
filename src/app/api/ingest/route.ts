...
const IngestPayloadSchema = z.object({
  ...
  vectorSearchLatencyMs: z.number().optional(),

  retrievedChunkCount: z.number().optional(),

  topSimilarityScore: z.number().optional(),

  retrievedDocumentIds: z.array(z.string()).optional(),

  retrievedPages: z.array(z.number()).optional(),
...
export async function POST(req: NextRequest) {
  try {
    ...
    const log = await InferenceLog.create({
      ...
      embeddingLatencyMs: payload.embeddingLatencyMs,

      vectorSearchLatencyMs: payload.vectorSearchLatencyMs,

      retrievedChunkCount: payload.retrievedChunkCount,

      topSimilarityScore: payload.topSimilarityScore,

      retrievedDocumentIds: payload.retrievedDocumentIds,

      retrievedPages: payload.retrievedPages,

      metadata: payload.metadata || {},
    });

    // Inference logs represent model calls, not messages. Message counts are
    // maintained by the chat flow when it persists user and assistant messages.
