...
export class InferenceLogger {
  ...
  async complete(
    ...
    try {
      ...
      const payload: InferenceLogPayload = {
        ...
        retrievalLatencyMs: options.retrievalLatencyMs,
        embeddingLatencyMs: options.embeddingLatencyMs,
        vectorSearchLatencyMs: options.vectorSearchLatencyMs,
        retrievedChunkCount: options.retrievedChunkCount,
        topSimilarityScore: options.topSimilarityScore,
        retrievedDocumentIds: options.retrievedDocumentIds,
        retrievedPages: options.retrievedPages,
      };

      if (this.config.asyncLogging) {
        void this.shipLog(payload);
      } else {
        await this.shipLog(payload);
      }
    }
  ...
  private async shipLog(payload: InferenceLogPayload): Promise<void> {
    try {
...
export function getInferenceLogger(
  overrides: Partial<InferenceLoggerConfig> = {},
): InferenceLogger {
  const config: InferenceLoggerConfig = {
  ...
  if (Object.keys(overrides).length > 0) {
    return new InferenceLogger(config);
  }

  if (!_loggerInstance) {
    _loggerInstance = new InferenceLogger(config);
  }
