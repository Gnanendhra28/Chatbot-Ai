...
const InferenceLogSchema = new Schema<IInferenceLog>(
  {
    ...
    vectorSearchLatencyMs: { type: Number },
    retrievedChunkCount: { type: Number },
    topSimilarityScore: { type: Number },
    retrievedDocumentIds: [{ type: String }],
    retrievedPages: [{ type: Number }],
...
InferenceLogSchema.index({ provider: 1, modelname: 1, createdAt: -1 });
